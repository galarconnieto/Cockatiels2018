...
for i in range(0, (len(ip_pi))):  #use this for more than one pi
    ...
    try:
		...
		#Folder of current day's videos
            #abc1= str("ssh " + ip_pi[i] + " mkdir -v" + " ./APAPORIS/TO_TRANSFER")
            #abc1= str("ssh " + ip_pi[i] + " df -h /")
            #abc1= str("ssh " + ip_pi[i] + " sudo pip install picamera")
	    #abc2= str("ssh " + ip_pi[i] + " sudo pip3 install picamera")
	    abc3= str("ssh " + ip_pi[i] + " sudo reboot")
            #print(abc1)
